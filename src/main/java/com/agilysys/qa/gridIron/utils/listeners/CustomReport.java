...
import com.agilysys.qa.gridIron.utils.annotations.PRIORITY;
...
public class CustomReport implements IReporter {
	...
	String Environment = Endpoints.getEnvironment();

	String JiraId =  Endpoints.getJiraId();

	SimpleDateFormat formatter = new SimpleDateFormat("dd-MM-yyyy-HH-mm-ss");
	Date date = new Date();

	private String fileName = "Customized Report-" + JiraId + "-" + Environment + "-" + formatter.format(date)
			+ ".html";

	private String reportTitle = "Customized Report for " + JiraId + " in " + Environment;
	 ...
	 * the method details
	 */
	protected void generateMethodSummaryReport(List<ISuite> suites)throws JSONException {
		m_methodIndex = 0;
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r2 : r.values()) {
				...
				updateResult(testContext.getFailedTests(), "2");
				updateResult(testContext.getPassedTests(), "1");
	...
	private void updateResult(IResultMap tests, String status) throws JSONException {
		...
		for (ITestResult result : testResultsList) {
			...
			TestCase testCaseAnnotation = method.getConstructorOrMethod().getMethod().getAnnotation(TestCase.class);
			if (testCaseAnnotation != null) {
				String jiraKey = testCaseAnnotation.id();
			...
			for (ITestNGMethod method : getMethodSet(tests, suite)) {
				...
				buff.append(
						...
						"<td>" + (jira != null ? jira.id().toString() : "-") + " : "
								...
								+ "<td class=\"numi\" style=\"text-align:left;padding-right:2em\">" + firstLine
								+ "<br/>"
								+ (null == Screenshot ? ""
	...
	public void generateSuiteSummaryReport(List<ISuite> suites) {
		...
		if (status == qty_pass_m) {
			summaryCell("Pass", "background-color:green", true);
		} else {
			summaryCell("Fail", "background-color:red", true);
